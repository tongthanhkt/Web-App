...
//LECTURER
// CHANGE PROFILE.
function Update_Data_Lecturer(id, info, index) {
  return new Promise(function (res, rej) {
    if (info != "") {
      if (index == 0) {
        database.query(
          "Update Lecturer Set Fullname = ? WHERE LecturerID = ?",
          [info, id],
          function (error) {
            if (error) {
              return console.error(error.message);
        ...
        database.query(
          "Update Lecturer Set PhoneNumber = ? WHERE LecturerID = ?",
          [info, id],
          function (error) {
            if (error) {
              return console.error(error.message);
        ...
        database.query(
          "Update Lecturer Set StartYear = ? WHERE LecturerID = ?",
          [info, id],
          function (error) {
            if (error) {
              return console.error(error.message);
        ...
        database.query(
          "Update Staff Set DateOfBirth = ? WHERE LecturerID = ?",
          [info, id],
          function (error) {
            if (error) {
              return console.error(error.message);
        ...
        database.query(
          "Update Lecturer Set Faculty = ? WHERE LecturerID = ?",
          [info, id],
          function (error) {
            if (error) {
              return console.error(error.message);
...
exports.lecturer_change_profile = async (req, res) => {
  try {
    console.log(req.body);
    var id = localStorage.getItem("ID");
    console.log(id);
    ...
    if (!fullname && !phone && !year && !DoB && !faculty) {
      return res
        .status(400)
        .render("../views/lecturer/lecturer_change_profile", {
          message: "Provide at least 1 data!",
        });
    } else {
      if (phone.length > 0 && phone.length < 10) {
        ...
        for (var i = 0; i < data.length; i++) {
          // Assign variables to the same
          p = p.then(Update_Data_Lecturer(id, data[i], i));
        }
        return res
          .status(400)
          .render("../views/lecturer/lecturer_change_profile", {
...
// thay đổi mật khẩu
exports.lecturer_change_password = async (req, res) => {
  const password = localStorage.getItem("Password");
  const id = localStorage.getItem("ID");
  const { oldPassword, newPassword, passwordConfirm } = req.body;
  ...
  if (!oldPassword || !newPassword || !passwordConfirm) {
    ...
    return res
      .status(400)
      .render("../views/lecturer/lecturer_change_password", {
        message: "Please provide full necessary information.",
      });
  }
  ...
  database.query("Update Lecturer set Password = ? where LecturerID = ?", [
    hashedPassword,
    id,
  ]);
  localStorage.setItem("Password", newPassword);
  return res.status(400).render("../views/lecturer/lecturer_change_password", {
    message: "Changed successfully!",
  });
};
...
exports.view_grade = (req, res) => {
  ...
  try {
    ...
    database.getConnection((err, connection) => {
      ...
      database.query(
        "SELECT SubjectID, Semester, Year, Midterm, Final, Total, Class FROM Course WHERE StudentID = ?",
        [id],
        (err, rows) => {
          if (!err) {
            return res
              .status(400)
              .render("../views/student/view_grade", { rows });
          }
          console.log("The data from table: \n", rows);
        }
      );
