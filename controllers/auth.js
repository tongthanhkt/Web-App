...
function Update_Data_Lecturer (id, info, index) {
    return new Promise(function (res, rej) {
        if (info != '') {
            if (index == 0) {
                database.query (
                    "Update Lecturer Set Fullname = ? WHERE LecturerID = ?", [info, id], function (error) {
                        if (error) {
                            return console.error(error.message);   
                    }
                });
                ...
                database.query (
                    "Update Lecturer Set PhoneNumber = ? WHERE LecturerID = ?", [info, id], function (error) {
                        if (error) {
                            return console.error(error.message);   
                    }
                });
                ...
                database.query (
                    "Update Lecturer Set StartYear = ? WHERE LecturerID = ?", [info, id], function (error) {
                        if (error) {
                            return console.error(error.message);   
                    }
                });
                ...
                database.query (
                    "Update Staff Set DateOfBirth = ? WHERE LecturerID = ?", [info, id], function (error) {
                        if (error) {
                            return console.error(error.message);   
                    }
                });
                ...
                database.query (
                    "Update Lecturer Set Faculty = ? WHERE LecturerID = ?", [info, id], function (error) {
                        if (error) {
                            return console.error(error.message);   
                    }
                });
...
exports.lecturer_change_profile = async (req, res) => {
    try {
        ...
        if (!fullname && !phone && !year && !DoB && !faculty){
            return res.status(400).render('../views/lecturer/lecturer_change_profile', {
                message: 'Provide at least 1 data!'
            })
        }
        else {
...
exports.lecturer_change_password = async(req, res) => {
    ...
    database.query('Update Account set Password = ? where ID = ?', [hashedPassword, id]);
    database.query('Update Lecturer set Password = ? where LecturerID = ?', [hashedPassword, id]);
    localStorage.setItem("Password", newPassword);
    return res.status(400).render('../views/lecturer/lecturer_change_password', {
        message: 'Changed successfully!'
    })
